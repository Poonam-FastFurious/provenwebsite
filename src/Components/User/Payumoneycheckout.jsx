...
const Payumoneycheckout = () => {
  ...
  const paymentHandler = async (e) => {
    ...
    const orderId = await createOrder(); // Function to create order
    const userId = localStorage.getItem("userid"); // Retrieve customer ID from localStorage again for payment
    ...
    const paymentDetails = {
      ...
      productinfo: "Order Payment", // Adjust as needed
      firstname: "Customer Name", // Replace with actual customer name
      email: "customer@example.com", // Replace with actual customer email
      userId,
    };

    const response = await fetch(Baseurl + "/api/v1/payments/create_payments", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    if (response.ok) {
      ...
      // Create the form for PayUmoney and submit it
      ...
      const params = {
        ...
        email,
        hash,
        surl: Baseurl + "/api/v1/payments/success", // Backend success URL
        furl: Baseurl + "/api/v1/payments/failed", // Backend failure URL
      };
...
export default Payumoneycheckout;
